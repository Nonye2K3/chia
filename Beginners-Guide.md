...
In Chia, the only electricity required is the electricity to create the plots, and to run the hard drives. A hard drive can take 10 watts to power, plus its CPU power.  In comparison Blockchains like Bitcoin and Ethereum have to use multiple CPU's ( 300W each GPU est) , or much more for ASICs (1200 W est) to process a single user transaction.  The hash formulas that are transacted are single use.  All the electricity used was for a single transaction. Therefore Chia will use vastly less electricity as each proof created last approx 5 years of continuous use, on a hard drive using 10 watts.
