1. To plot parallel in Windows you will need to use powershell. A generic example for Beta 23:
```
cd C:\Users\yourUserName\AppData\Local\Chia-Blockchain\app-0.1.23\resources\app.asar.unpacked\daemon\
start-process ./chia.exe -argumentlist "plots create yourParametersGoHere"
start-process ....
....
```

A specific example, again using Beta 23:

```
cd C:\Users\yourUserName\AppData\Local\Chia-Blockchain\app-0.1.23\resources\app.asar.unpacked\daemon\
start-process ./chia.exe -argumentlist "plots create -k 32 -b 4000 -u 128 -r 4 -t d:\tempdrive1 -2 e:\tempdrive2 -d F:\plots -n 1"
```
If you are using Beta 16 you would change the `Chia-Blockchain\app-0.1.23\resources\` section to `Chia-Blockchain\app-0.1.16\resources\` in the command above.
...
3. You configuration and logs are found in `~\.chia\VERSION\log` and `~\.chia\VERSION\config` so `~\.chia\beta-1.0b23\log` in Beta 23. You can tail your logs with `Get-Content ~\.chia\VERSION\log\debug.log -wait`. To see more of what is going on, set your log level in `conf\config.yaml` to INFO from WARNING and restart. You can also use `\.chia.exe configure --set-log-level INFO from the app directory outlined above and then restart for the changes to take effect.
...
5. If you attempt to run more than one node on your local network, having uPnP on on both will cause both nodes significant confusion. You will need to use powershell to disable uPnP on all but one. An example for Beta 23:
```
cd C:\Users\yourUserName\AppData\Local\Chia-Blockchain\app-0.1.23\resources\app.asar.unpacked\daemon\
