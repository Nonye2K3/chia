...
You can start with the Command Line Interface (CLI) by checking the commands available in `~\AppData\Local\Chia-Blockchain\app-1.1.0\resources\app.asar.unpacked\daemon\`. Try `.\chia -h` or `.\chia plots -h` for example:
...
2. Change Directory `cd` 
	...
	On *PowerShell* type `cd $env:localAPPDATA\Chia-Blockchain\app-1.1.0\resources\app.asar.unpacked\daemon\` and press the enter key.
