...
Note:
Currently (beta15), the GUI doesn't show harvester plots. The best way to see if it's working is to check the log on your main machine `~/.chia/beta-1.0bx/log/debug.log` and see if you get messages like `[time stamp] harvester src.harvester           : WARNING  KeyError plot [directory of your harvester + plot file name] does not exist.` That means it knows you have a plot that has a proof that can solve the challenge hash, but it can't find the file locally. This shows it's reading the information from your harvesters properly, knowing the file exists somewhere in your harvester network.